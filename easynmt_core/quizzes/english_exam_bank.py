...
def _rubric(instruction: str, task: str, parts: tuple[tuple[str, ...], tuple[str, ...], tuple[str, ...]], *, skill: str, note: str, source_text: str = "", placeholder: str = "1) ...\n2) ...\n3) ...") -> QuestionBlueprint:
    ...
    return QuestionBlueprint(
        instruction=instruction,
        task=task,
        answer_format="Запиши три відповіді з нового рядка. Кожна правильна частина дає 1 бал.",
        answer_type="long_text",
        correct_answer=answer,
        accepted_answers=(answer,),
        scoring_parts=parts,
        explanation=f"Еталон:\n{answer}\n{note}",
        feedback_hint="Перевір кожен рядок окремо: у завданні є три незалежні бали.",
        grading_mode="rubric",
        points=3,
        skill=skill,
        source_text=source_text,
