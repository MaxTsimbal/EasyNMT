(() => {
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        const renderConversation = ({ preserveScroll = false } = {}) => {
            ...
            if (elements.activeSubtitle) {
                ...
                elements.activeSubtitle.textContent = messageCount
                    ...
                    : (conversation.context?.lessonTitle || "Easy Chat v2.0");
            }
            renderHistory();
        ...
        const updateInputState = () => {
            ...
            input.style.height = `${Math.min(188, Math.max(26, input.scrollHeight))}px`;
        ...
        const openRenameDialog = (conversationId) => {
            ...
            if (elements.renameDialog?.showModal) {
                ...
                window.setTimeout(() => elements.renameInput?.select(), 20);
            }
        };
        ...
        elements.input?.addEventListener("focus", () => window.setTimeout(() => scrollToBottom("smooth"), 90));
        ...
        window.visualViewport?.addEventListener("resize", () => {
            syncViewport();
            if (document.activeElement === elements.input) requestAnimationFrame(() => scrollToBottom());
        });
